  ...
  <div class="work-detail">
    ...
    <div class="work-detail-content" style="padding-bottom: 65px;">
      ...
      <p v-if="groupInfo" class="work-detail-row">
        <span class="work-detail-label">秩序分组：{{ groupInfo.name }}</span>
      </p>
      <!--设备信息-->
...
import { minipGuardianTaskInfo } from '@/api/task'
  ...
  methods: {
    ...
    getGuardianTaskDetail () {
      minipGuardianTaskInfo({
        ...
        if (res.code === 200) {
          ...
          this.groupInfo = this.taskInfo['guardian_group'] || {}
          return
        }
        this.$toast('获取任务详情失败')
      })
    ...
    toReport (item) {
      ...
      this.$router.push({
        ...
        query: {
          source: source, commitId: item.commit_id, pointId: item.id, taskId: this.taskInfo.id, type: WorkOrderType.squence, groupId: this.flowInstance.group_id
        }
      })
    }
  }
