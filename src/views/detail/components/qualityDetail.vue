<template>
  ...
  <div class="work-detail">
    <div class="work-detail-content">
      ...
      <!--计划来源-->
      <p class="work-detail-row" v-show="flowInstance.source===21">
      ...
      <!--计划周期-->
      <div class="work-detail-row" style="align-items:flex-start" v-show="flowInstance.source===21">
    ...
    <div class="work-detail-content" style="padding-bottom: 65px;">
      ...
      <p v-if="groupInfo && groupInfo.group_type === 1" class="work-detail-row">
        <span class="work-detail-label">品质分组：{{ groupInfo.name }}</span>
      </p>
      <!--设备信息-->
      <ul class="tack-content">
        <li v-for="(item, index) in groupInfo.location_points" :key="index" class="track-item">
          <div class="detail">
            <!--待处理-->
            ...
            <template v-if="item.commit_id && item.is_right">
              <van-cell class="done" :title="item.name" is-link>
                <template #icon>
                  <svg-icon icon-class="checked-blue"></svg-icon>
                </template>
                <span @click="showListDetails(item)">正常</span>
              </van-cell>
            </template>
            ...
            <template v-if="item.commit_id&&!item.is_right">
              <van-cell class="done" is-link>
                <template #title>
                  <span @click="showListDetails(item)">{{ item.name }}</span>
                </template>
                <template #icon>
                  <svg-icon icon-class="checked-blue"></svg-icon>
                </template>
                <span v-if="item.repair_log_id" class="error" style="color: #F7B500;" @click="showListDetails(item)">异常，已提单</span>
                <span v-else class="error" @click.stop="toReport(item)">异常，去提单</span>
              </van-cell>
            </template>
          </div>
        </li>
...
import { minipQualityTaskInfo } from '@/api/task'
...
export default {
  ...
  watch: {
    'orderId': {
      handler (val) {
        val && this.getGuardianTaskDetail()
      },
      immediate: true
    }
  },
  ...
  methods: {
    ...
    // 获取设备任务详情
    getGuardianTaskDetail () {
      minipQualityTaskInfo({
        work_order_record_id: this.orderId
      }).then(res => {
        if (res.code === 200) {
          this.taskInfo = res.data || {}
          this.groupInfo = this.taskInfo['quality_group'] || {}
    ...
    toComplate (item, index, oId) {
      ...
      const dealIndexs = []
      const points = this.groupInfo['location_points'] || []
      for (let i = 0; i < points.length; i++) {
        if (!points[i].commit_id) {
          dealIndexs.push(i)
        }
      }

      // 判断是否有待处理的任务
      if (!dealIndexs || !dealIndexs.length) {
        this.$router.push({
          name: 'WorkDeal',
          query: { id: oId },
          params: {
        ...
        // this.$toast('没有待处理的任务哦~')
        return
      }

      if (item && index) {
        if (dealIndexs.indexOf(index) > 0) {
          this.$confirm({
            message: `本点位前还有个${dealIndexs.indexOf(index)}没有处理，确认继续？`,
            confirmButtonColor: '#BC8D58'
          }).then(() => {
            this.goDeal(points[index])
          }).catch(() => {})
          return
        }
        this.goDeal(item)
    ...
    goDeal (item) {
      ...
      const query = { orderId: this.orderId }
      if (item && item.id) {
        query.point_id = item.id
        // query.group_type = this.groupInfo.group_type
      }
    ...
    // 去提单
    toReport (item) {
      const source = WorkOrderSource.qualityTask
      this.$router.push({
        name: 'WorkBrokenReport',
        query: {
          source: source, commitId: item.commit_id, pointId: item.id, taskId: this.taskInfo.id, type: WorkOrderType.quality, groupId: this.flowInstance.group_id, group_type: this.groupInfo.group_type
        }
      })
    }
  }
