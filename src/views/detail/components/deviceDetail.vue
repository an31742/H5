<template>
  ...
  <div class="work-detail">
    <div class="work-detail-content">
      ...
      <!--计划类型-->
      <p class="work-detail-row">
        <span class="work-detail-label">计划类型：</span>
        <span class="work-detail-desc">
          {{ taskInfo.project | projectType }}
        </span>
      </p>
      <!--计划名称-->
      <p class="work-detail-row">
    ...
    <div class="work-detail-content" style="padding-bottom: 65px;">
      ...
      <p v-if="groupInfo" class="work-detail-row">
        <span class="work-detail-label">工程分组：{{ groupInfo.name }}</span>
      </p>
      <!--设备信息-->
      <ul class="tack-content">
        <li v-for="(item, index) in groupInfo.devices" :key="index" class="track-item">
          <div class="detail">
            <!--待处理-->
            ...
            <template v-if="item.commit_id && item.is_right">
              <van-cell class="done" :title="item.name" is-link>
                ...
                <span @click="showListDetails(item, index)">正常</span>
              </van-cell>
            </template>
            ...
            <template v-if="item.commit_id&&!item.is_right">
              <van-cell class="done" is-link>
                ...
                <span v-if="item.repair_log_id" class="error" style="color: #F7B500;" @click="showListDetails(item, index)">异常，已提单</span>
...
import { minipDeviceTaskInfo } from '@/api/task'
...
export default {
  ...
  methods: {
    ...
    getDeviceTaskDetail () {
      minipDeviceTaskInfo({
        ...
        if (res.code === 200) {
          ...
          this.groupInfo = this.taskInfo['eng_group'] || {}
          return
        }
        this.$toast('获取任务详情失败')
      })
    ...
    toComplate (item, index, oId) {
      const dealIndexs = []
      const devices = this.groupInfo.devices || []
      for (let i = 0; i < devices.length; i++) {
        if (!devices[i].commit_id) {
          dealIndexs.push(i)
        }
      }
      ...
      if (item && index) {
        if (dealIndexs.indexOf(index) > 0) {
          this.$confirm({
            message: `本设备前还有个${dealIndexs.indexOf(index)}没有处理，确认继续？`,
            confirmButtonColor: '#BC8D58'
          }).then(() => {
            this.goDeal(devices[index])
          }).catch(() => {})
          return
        }
        this.goDeal(item)
    ...
    showListDetails (item) {
      this.$router.push({
        ...
        query: { id: item.commit_id, project: this.taskInfo.project, taskId: this.taskInfo.id }
      })
    },
    ...
    toReport (item) {
      ...
      this.$router.push({
        ...
        query: {
          source: source, commitId: item.commit_id, deviceId: item.id, taskId: this.taskInfo.id, type: WorkOrderType.device, groupId: this.flowInstance.group_id
        }
      })
    }
  }
