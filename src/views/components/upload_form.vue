<template>
  <div class="upload-wrap">
    <van-uploader
      v-model="fileItems"
      name="file"
      upload-icon="plus"
      :max-count="max"
      :disabled="disabled"
      :before-read="beforeRead"
      :after-read="afterRead"
      :style="uploadStyle"
      :deletable="deletable"
      :previewOptions="{ closeable: true }"
      :preview-image="previewImage"
      @delete="deleteFile"
      :capture="capture"
      :accept="accept"
    >
      <slot></slot>
      <template #preview-cover="item">
        <slot name="preview-cover" :item="item">
        </slot>
      </template>
    </van-uploader>

    <div v-if="!hideTips" class="upload-tips">(单张照片不超过{{ maxPerSize }}M，最多只可上传{{ max }}张)</div>
  </div>
</template>

<script>
import COS from 'cos-js-sdk-v5'
import Exif from 'exif-js'
import { isIos, isAndroidApp } from '@/utils/index'
import Compressor from 'compressorjs'
import { getTecentUploadToken } from '@/api/common'

export default {
  name: 'UploadComponent',
  props: {
    accept: { // 允许文件类型
      type: String,
      default: () => 'image/*'
    },
    capture: { // 模式，可选 camera
      type: String,
      default: () => null
    },
    max: { // 最大上传张数
      type: Number,
      default: () => 100
    },
    uploadText: { // 提示文案
      type: String,
      default: () => '上传图片'
    },
    hideTips: { // 是否隐藏底部提示信息(单张不超过...)
      type: Boolean,
      default: () => false
    },
    disabled: { // 是否禁用
      type: Boolean,
      default: false
    },
    fileList: { // 回填图片
      type: [Array, String],
      default () {
        return []
      }
    },
    previewOptions: { // 预览配置
      type: Object,
      default: () => {}
    },
    uploadStyle: { // 最外层div样式
      type: String,
      default: 'padding: 16px 10px 6px 16px'
    },
    checkTimeStamp: { // 是否校验照片的拍照时间
      type: Number,
      default: () => null
    },
    deletable: {
      type: Boolean,
      default: false
    },
    previewImage: {
      type: Boolean,
      default: true
    }
  },
  data () {
    return {
      androidAttrs: undefined,
      file: null,
      curIndex: 0,
      fileItems: [],
      loading: false,
      maxPerSize: 20 // 单张照片最大尺寸（单位: M）
    }
  },
  computed: {
    /*
    * 已经上传的图片列表
    *   这里之所以要向外emit属性uploadedPictures，不直接emit属性fileItems
    *   是因为：如果直接给fileItems赋值url，页面上的预览就会去网络请求url，可能会出现"暂时无法查看"或者"加载失败"的情况
    *   所以：fileItems里不给赋值url，预览本地的blob，但是向外emit的时候，需要把腾讯云上的图片url带上
    * */
    uploadedPictures () {
      const arr = JSON.parse(JSON.stringify(this.fileItems)) || []

      return arr.map(item => {
        item.url = item.image_url || item.url
        item.orgUrl = item.orgUrl || item.url

        // 删除属性 content file message image_url
        const listKey = ['content', 'file', 'message', 'image_url']
        listKey.forEach(key => {
          delete item[key]
        })

        return item
      })
    }
  },
  watch: {
    fileList (val) {
      // 这里要先判断下，新旧的列表url是否相同，否则可能会引起循环监听
      if (!this.isSamePictures(val || [], this.fileItems)) {
        this.fileItems = this.initFileItems(val)
        this.$emit('change', this.uploadedPictures)
      }
    }
  },
  created () {
    this.fileItems = this.initFileItems(this.fileList)
    this.$emit('change', this.uploadedPictures)
    // 在页面一进来时就去判断当前使用机型
    const model = navigator.userAgent
    // 判断是否是安卓手机，是则是true
    this.androidAttrs = model.indexOf('Android') > -1 || model.indexOf('Linux') > -1
    console.log(999999)
    console.log('是否是安卓设备')
    console.log(isAndroidApp())
    console.log(navigator.userAgent)
  },
  methods: {
    isAndroidApp,
    // 判断两组图片是否一样
    isSamePictures (arr = [], list = []) {
      const a = arr.filter(i => i).map(item => item.image_url || item.url || item.orgUrl || item)
      const b = list.filter(i => i).map(item => item.image_url || item.url || item.orgUrl || item)

      return a.toString() === b.toString()
    },
    // 传入的初始值
    initFileItems (arr = []) {
      if (typeof arr === 'string') {
        try {
          arr = JSON.parse(arr)
        } catch (e) {
          arr = []
        }
      }

      return arr.map(item => {
        if (item && item.url) {
          return item
        }

        item = { url: item, name: this.getQueryString(item, 'filename'), status: 'done', orgUrl: item }

        return item
      })
    },
    // 是否是图片
    isImageType (type) {
      return /^image\/.+$/.test(type)
    },
    // 按照下标删除
    deleteFileByIndex (index) {
      this.fileItems.splice(index, 1)
      this.$emit('change', this.uploadedPictures)
    },

    // 上传前校验
    beforeRead (file) {
      if (this.loading) {
        this.$toast('请等待上传完成')
        return false
      }

      const acc = this.accept === 'image/*' ? 'image/jpg,image/png,image/jpeg' : this.accept
      const type = file.type.substr(file.type.indexOf('/') + 1)
      // 校验后缀
      if (acc && (!type || acc.indexOf(type) < 0)) {
        this.$toast('请上传' + acc.replace(/image\//g, '') + '后缀的文件')
        return false
      }
      // 校验尺寸
      if (file.size > this.maxPerSize * 1024 * 1024) {
        this.$toast('请上传不大于' + this.maxPerSize + 'M的文件')
        return false
      }

      // 校验照片的拍摄时间
      // 由于ios下拿不到照片的拍摄时间，所以在ios下自动忽略
      // android下，能拿到时间的图片做时间校验，拿不到的做通过降级处理
      if (this.checkTimeStamp && this.checkTimeStamp > 0 && !isIos()) {
        const that = this
        return new Promise((resolve, reject) => {
          Exif.getData(file, function () {
            const res = Exif.getAllTags(this)
            let time = res.DateTimeOriginal || ''
            time = time.replace(/(\d{4}):(\d{2}):(\d{2})/, '$1/$2/$3')
            if (time && new Date(time)) {
              const expire = new Date().getTime() - new Date(time).getTime()
              if (expire / 1000 / 60 > that.checkTimeStamp) {
                that.$toast(`请上传${that.checkTimeStamp}分钟内拍摄的图片`)
                reject(new Error(`请上传${that.checkTimeStamp}分钟内拍摄的图片`))
              }
            }
            resolve(file)
          })
        })
      }

      return true
    },
    // 删除
    deleteFile (file) {
      this.file = null
      this.curIndex = 0
      this.$emit('change', this.uploadedPictures)
    },
    // 文件上传
    afterRead (file, detail) {
      this.curIndex = detail.index
      file.status = 'uploading'
      this.loading = true
      this.$emit('loading', true)
      this.file = file.file

      // 只图片需要压缩
      if (this.isImageType(this.file.type)) {
        this.compressImage(file)
      } else {
        this.handleUpload(file)
      }
    },
    // 压缩图片
    compressImage (cur) {
      const that = this
      const { file } = this
      const maxSize = 2097152
      const fileSize = file.size

      // 不超过2M不压缩
      if (fileSize < maxSize) {
        that.handleUpload(cur)
        return
      }

      // 图片压缩处理标准：1.最大宽度1080 2.最大2M
      const quality = maxSize / fileSize
      const { handleUpload } = this
      const ci = new Compressor(file, {
        // maxWidth: 1080,
        quality: Math.sqrt(quality),
        success (res) {
          that.file = res
          handleUpload(res)
        },
        error (err) {
          console.warn('压缩图片失败，使用原图上传', err)
          handleUpload(cur)
        }
      })
      console.log('ci', ci)
    },
    // 预加载外网图片
    loadOuterImage (url) {
      const img = new Image()
      img.src = url
      return new Promise((resolve, reject) => {
        if (img.complete) {
          resolve(url)
        }
        img.onload = (image) => {
          console.log('onloadUrl', url)
          resolve(url)
        }
        img.onerror = (url) => {
          reject(url)
        }
      })
    },
    // 上传文件
    handleUpload (cur) {
      let cos = null
      const that = this
      const { file } = this
      const suffix = file.name.substr(file.name.lastIndexOf('.') + 1)

      const startUpload = (res) => {
        cos.putObject({
          Bucket: res.bucket || 'fpms-1301854395',
          Region: res.region || 'ap-nanjing',
          Key: res.filekey,
          Body: that.file,
          onProgress: async function (progress) {
            if (progress && progress.percent === 1) {
              cur.status = 'done'
              that.loading = false
              that.$emit('loading', false)

              // url后面加参数filename，方便回填的时候显示
              const name = that.file && that.file.name || ''
              const url = `${res.url}/${res.filekey}?imageMogr2/thumbnail/!50p&filename=${encodeURIComponent(name)}`

              await that.loadOuterImage(url).then((str) => {
                that.fileItems[that.curIndex] = {
                  image_url: str,
                  url: str,
                  name: name,
                  status: 'done',
                  orgUrl: `${res.url}/${res.filekey}?filename=${encodeURIComponent(name)}`
                }
              }).catch(e => {
                that.fileItems[that.curIndex].image_url = url
                that.fileItems[that.curIndex].name = name
                that.fileItems[that.curIndex].status = 'done'
                that.fileItems[that.curIndex].orgUrl = `${res.url}/${res.filekey}?filename=${encodeURIComponent(name)}`
              })

              that.$emit('change', that.uploadedPictures)
            }
          }
        }, function (err) {
          if (err) {
            cur.status = 'failed'
            that.loading = false
            that.$emit('loading', false)
            that.$toast('文件上传失败，请重试')
          }
        })
      }

      getTecentUploadToken({ suffix }).then(res => {
        res = res.data || {}
        cos = new COS({
          getAuthorization: (opt, callback) => {
            const data = {
              TmpSecretId: res.token.TmpSecretId,
              TmpSecretKey: res.token.TmpSecretKey,
              XCosSecurityToken: res.token.Token,
              StartTime: res.start_time,
              ExpiredTime: res.expired_time
            }
            callback(data)
          }
        })
        console.log(res)
        startUpload(res)
      }).catch(_ => {
        cur.status = 'failed'
        that.loading = false
        that.$emit('loading', false)
      })
    }
  }
}
</script>

<style type="text/css" lang="scss">
  .upload-wrap {
    position: relative;background: #fff;font-family: PingFangSC-Regular,PingFang SC;
    .van-uploader__upload-icon { color: #BC8D58; }
    .van-uploader__upload-text { font-size: 12px;color: #BC8D58;line-height: 17px; }
    .van-uploader__wrapper {
      display: flex;
      .van-uploader__preview, .van-uploader__upload {
        width: 80px;height: 80px;background: #FAF7F4;border-radius: 4px;position: relative;margin: 0 8px 8px 0;
      }
      .van-uploader__preview-delete { top: 5px !important;right: 5px !important;opacity: 0.6;font-size: 18px;width: 16px;height: 16px;border-radius: 100%;border: 1px solid #fff; }
      .van-uploader__preview-delete-icon {
        transform: scale(1);
        top: 2px;
        left: 2px;
        font-size: 12px;
        right: 0;
        line-height: 12px;
      }
      .van-uploader__preview-image { width: 100%; height: 100%;border-radius: 4px; }
    }
    .dashed .van-uploader__upload { border: 1px dashed #ccc; }
    .upload-tips { font-size: 10px;font-weight: 400;color: #C4C4C4;line-height: 14px;padding-bottom: 8px; }
    .van-uploader__wrapper--disabled { opacity: 1; }
  }
</style>
