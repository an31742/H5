    ...
    <div class="search-centont">
      <div v-if="total > 0">
        ...
        <div class="search-main">
          <ul>
            <li v-for="(item,index) in data" :key="index" class="search-item" @click="linkDetail(item)"><span class="item-name" v-html="item.room_name"></span><span class="user-name" v-html="item.name"></span></li>
          </ul>
        </div>
      </div>
...
<script>
import { minipRoomSearch } from '@/api/search'
...
export default {
  ...
  methods: {
    ...
    linkDetail (item) {
      this.$router.push(`/management/detail/room?id=${item.room_id}`)
    },
    // 清空状态
    clearState () {
      this.searchKey = ''
    ...
    userSearch () {
      ...
      minipRoomSearch(params).then(res => {
        if (res.code === 200) {
          this.loading = false
