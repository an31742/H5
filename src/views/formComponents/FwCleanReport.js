import { getCleanTaskCheckListCommit } from './api'
import Vue from 'vue'
const vue = new Vue()
...
class FwCleanReport {
  ...
  getOpts () {
    const options = [
      { code: `${this.item.code}_name`, type: 'FwInput', name: '环境点名称', required: false, readonly: 1 },
  ...
  setFormModal () {
    getCleanTaskCheckListCommit({ id: this.commitId }).then(res => {
      if (res.code === 200) {
