...
export default {
  ...
  created () {
    ...
    if (this.model[this.opt.code]) {
      ...
      this.model[this.opt.code + '_desc'] = this.userData.name
      this.model[this.opt.code] = this.userData.id
    } else if (this.opt.value) {
      // 存在默认值的情况
      this.selected = [].concat(this.opt.value)

      this.model[this.opt.code + '_desc'] = this.opt.value_desc
      this.model[this.opt.code] = this.opt.value
    }
  ...
  methods: {
    ...
    confirmSelected (arr, list = []) {
      ...
      // 单选时，返回int，而不是array
      if (this.opt.props && this.opt.props.mode && this.opt.props.mode === 'single') {
        this.$set(this.model, this.opt.code, this.selected[0] ? this.selected[0] : '')
      } else {
        this.$set(this.model, this.opt.code, this.selected.map(t => `${t}`))
      }
