<template>
  <div class="item-space" :class="{'is-view': mode === 'view', 'disabled': formReadonly(opt)}">
    <van-field
      v-model="selfUpload"
      clickable
      name="selfUpload"
...
export default {
  ...
  data () {
    return {
      selfUpload: '',
      maxCount: this.opt && this.opt.props && this.opt.props.max || 5
    }
  },
  computed: {
    // 展示数量
    ...
    // 回填
    initFiles () {
      if (this.model && this.model[this.opt.code] && Array.isArray(this.model[this.opt.code])) {
        return this.model[this.opt.code].filter(i => i)
      }
      return []
    }
  },
  methods: {
    ...
    changeFiles (list = []) {
      ...
      this.selfUpload = (arr && arr.length && JSON.stringify(arr)) || null
...
<style lang="scss" scoped>
  .item-space {
    ...
    &.disabled {
      ::v-deep {
        .van-uploader__input-wrapper, .van-uploader__preview-delete {
          display: none;
        }
      }
    }
