...
import { minipQualityTaskCheckListGet, minipQualityTaskCheckListCommit, minipQualityTaskInfo } from '@/api/task'
...
export default {
  ...
  data () {
    return {
      ...
      groupType: this.$route.query.group_type || ''
    }
  },
  computed: {
    // 下一步的文案
  ...
  methods: {
    ...
    rectification () {
      ...
      this.$router.replace({
        ...
        query: {
          source: source, pointId: this.pointId, taskId: this.taskId, type: WorkOrderType.quality, nextIndex: this.nextIndex, groupId: this.taskInfo.belong_group_id, group_type: this.groupType
        }
    ...
    getTaskInfo () {
      minipQualityTaskInfo({
        work_order_record_id: this.recordId
      }).then(res => {
        if (res.code === 200) {
          this.taskInfo = res.data || {}
          console.log('this.taskInfo', this.taskInfo)
          const gp = this.taskInfo['quality_group'] || {}
    ...
    // 获取检查项
    getCheckList () {
      minipQualityTaskCheckListGet({
        id: this.pointId,
        group_type: Number(this.groupType)
      }).then(res => {
        if (res.code === 200) {
    ...
    handleSubmit () {
      ...
      minipQualityTaskCheckListCommit(data).then(res => {
        if (res.code === 200) {
          if (this.nextIndex !== -1) {
            this.$toast('提交成功')
          } else {
          ...
          if (!res.data.is_right) {
            this.$confirm({
              ...
              this.$router.replace({
                name: 'WorkBrokenReport',
                query: {
                  source: source, commitId: res.data.id, pointId: this.pointId, taskId: this.taskId, type: WorkOrderType.quality, nextIndex: this.nextIndex, groupId: this.taskInfo.belong_group_id, group_type: this.groupType
                }
              })
            }).catch(() => {
              fun()
