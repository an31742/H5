...
import { minipGuardianCheckListGet, minipGuardianCheckListCommit, minipGuardianTaskInfo } from '@/api/task'
...
export default {
  ...
  methods: {
    ...
    getTaskInfo () {
      minipGuardianTaskInfo({
        ...
        if (res.code === 200) {
          ...
          const gp = this.taskInfo['guardian_group'] || {}
    ...
    handleSubmit () {
      ...
      minipGuardianCheckListCommit(data).then(res => {
        if (res.code === 200) {
          if (this.nextIndex !== -1) {
            this.$toast('提交成功')
          } else {
          ...
          if (!res.data.is_right) {
            this.$confirm({
              ...
              this.$router.replace({
                ...
                query: {
                  source: source, commitId: res.data.id, pointId: this.pointId, taskId: this.taskId, type: WorkOrderType.squence, nextIndex: this.nextIndex, groupId: this.taskInfo.belong_group_id
                }
              })
            }).catch(() => {
              fun()
    ...
    rectification () {
      ...
      this.$router.replace({
        name: 'WorkBrokenReport',
        query: {
          source: source, pointId: this.pointId, taskId: this.taskId, type: WorkOrderType.squence, nextIndex: this.nextIndex, groupId: this.taskInfo.belong_group_id
        }
      })
    }
  }
