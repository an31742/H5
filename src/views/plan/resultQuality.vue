<template>
  <div class="plan-result result">
    ...
    <div class="result-content">
      ...
      <p class="result-gray result-mgb8">{{ taskItemInfo.standard }}</p>
...
<script>
import formResult from 'views/components/formResult'
import { minipQualityTaskCheckListCommitGet } from 'api/task'
...
export default {
  ...
  methods: {
    // 获取提交记录
    getCommitDetail () {
      minipQualityTaskCheckListCommitGet({
        id: this.commitId
      }).then(res => {
        if (res.code === 200) {
          ...
          this.pointId = this.commitDetail.location_point_id || ''
    ...
    toReport () {
      ...
      this.$router.push({
        ...
        query: {
          source: source, commitId: this.commitId, pointId: this.pointId, taskId: this.taskId, type: WorkOrderType.quality, groupId: this.commitDetail.belong_group_id, group_type: this.commitDetail.group_type
        }
      })
    }
  }
