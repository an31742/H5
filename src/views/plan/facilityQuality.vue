  ...
  <div class="facility">
    ...
    <div class="facility-button">
      ...
      <template v-else>
        <!--扫码签到-->
        <van-button
          v-if="taskInfo.checkin_scan_code && !hasCheckQrcode && groupType === 1"
          style="font-size:18px;height:40px;"
          round
          block
          type="primary"
        ...
        <van-button
          v-else-if="taskInfo.checkin_scan_code && !hasCheckQrcode && groupType === 2"
          style="font-size:18px;height:40px;"
          round
          block
          type="primary"
...
export default {
  ...
  mounted () {
    ...
    this.groupType = this.$route.query.group_type || ''
    this.init()
  },
  methods: {
    // 初始化
    init () {
      ...
      minipQualityTaskInfo(params).then(res => {
        if (res.code === 200) {
          this.taskInfo = res.data || {}
          // eslint-disable-next-line
          this.groupType = this.taskInfo.quality_group.group_type
          const { location_points } = this.taskInfo.quality_group
