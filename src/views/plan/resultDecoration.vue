<template>
  <div class="plan-result result">
    ...
    <div class="result-button">
      <!--设备异常，去提单-->
      <template v-if="commitDetail.is_right===0 && !commitDetail.question_id">
        <span class="result-button-cancel">不合格</span>
        <van-button
          class="report-btn"
          round
          block
      ...
      <!--设备异常，已提单-->
      <template v-if="commitDetail.is_right===0 && commitDetail.question_id">
        <span class="result-button-error">不合格，问题已提报</span>
      </template>

      <!--设备正常-->
      <template v-if="commitDetail.is_right===1">
        <span class="result-button-normal">合格</span>
      </template>
    </div>
  </div>
</template>
