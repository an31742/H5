        ...
        <template v-if="list.length>0">
          <div class="approve-list">
            <div v-for="(item, index) in list" :key="index" class="approve-item" @click="viewDetail(item)">
              ...
              <p class="approve-text" v-if="type == 2 || type == 0">状态：{{ getNameByValue(receiveStatus, item.status, 'label') }}</p>
              <p class="approve-text">申请单号：{{ item.series || '--' }}</p>
              <template v-if="type == 2">
                <p class="approve-text">申请人：{{ item.apply_user_name || '--' }}</p>
                <p class="approve-text">使用人：{{ item.use_user_name || '--' }}</p>
              </template>
