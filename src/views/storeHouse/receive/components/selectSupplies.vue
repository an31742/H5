<template>
  <div class="service-wrap flex-column" style="margin-top: 101px">
    <div class="expand">
...
<script>
import { assetsGroupList, levelTree } from '@/api/storeHouse'
...
export default {
  ...
  methods: {
    ...
    getList (serviceItem, subServiceItem) {
      this.loading = true
      assetsGroupList(this.apiParams).then(res => {
        this.loading = false
        if (res.code === 200) {
        ...
        if (res.code === 200) {
          ...
          const format = function (list) {
            list.forEach(j => {
              j.label = j.name
              j.value = j.id
              j.children && j.children.forEach(j => {
                j.label = j.name
                j.value = j.id
