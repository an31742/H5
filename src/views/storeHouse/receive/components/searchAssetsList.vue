    ...
    <div v-else class="search-centent">
      <div v-if="total > 0" style="padding-bottom: 1.92rem">
        <div class="search-main">
          <div v-for="(item, index) in list" :key="index" class="search-item" :class="item.id === searchId? 'search-item-light' : ''">
  ...
  methods: {
    ...
    startSearch () {
      ...
      this.timer = setTimeout(() => {
        if (this.searchFun) {
          const arr = this.searchFun(
            this.copyArray(this.allData),
            this.searchKey
          )
    ...
    getUserList () {
      ...
      this.api(params).then((res) => {
        this.loading = false
        if (res.code === 200) {
          this.list = this.formatFun
            ? this.formatFun(res.data || [])
            : res.data.list || []
