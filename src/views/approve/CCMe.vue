<template>
  <div class="ccme flex-column h100">
    <div class="ccme-header" style="z-index: 10">
      <van-search
        v-model="searchValue"
        show-action
    ...
    <list ref="al" :api="flowInstanceCCList" :params.sync="searchParams" @viewDetail="viewDetailTap"></list>
  </div>
</template>
...
import { flowInstanceCCList } from '@/api/approve'
  ...
  methods: {
    ...
    viewDetailTap (val) {
      ...
      this.$router.push({
        name: 'ApproveDetail',
        query: {
          instanceId: val['flow_instance'].record_id,
          noticeId: val['flow_cc'].id
        }
      })
    },
    // 刷新列表
