  ...
  <div class="work-report flex-column">
    ...
    >
      ...
      <FwHouse
        v-if="showRoom"
        :model="formModel"
        :opt="{ user_id: (launcher || '').replace('C#', ''), code: 'room_id', name: '选择房屋', required: true }"
        @confirm="selectRoom"
      />
...
<script>
import { wfeFlowInstanceLaunch, wfeFlowInstanceLaunchForm, costAdd } from '@/api/wfe'
  ...
  methods: {
    ...
    addCost (item) {
      const data = {
        ...
        amount: this.formModel[item.code + '_service_fee'] + this.formModel[item.code + '_material_fee'],
        relationships: this.getRelationships(this.formModel[item.code + '_service'], this.formModel[item.code + '_material'])
      }
      costAdd(data).then(res => {
        console.log(res)
      })
    },
    // 表单提交
    async handleSubmit () {
      ...
      // 有偿工单的特殊操作：处理完当前节点后，“是否收费”这个表单一律不许编辑
      this.options.forEach(item => {
        if (item.code === 'srv_prop_is_cost') {
          item.readonly = 1
          // this.allCost()
          if (this.formModel[item.code] === 1) {
            // 需要生成明细
            this.addCost(item)
          }
        }
      ...
      // 有偿工单需要传递room_id
      if (this.formModel.srv_select_room) {
        this.room_id = this.formModel.srv_select_room
      }
